...
final class WindowHelper implements Application.ActivityLifecycleCallbacks {
    ...
    WindowHelper(ToastHelper helper, Activity activity) {
        mToastHelper = helper;
        mCurrentTag = getObjectTag(activity);
        mActivitySet.put(mCurrentTag,activity);
        Application currentApplication = activity.getApplication();
        currentApplication.registerActivityLifecycleCallbacks(this);
    }
